...
local _package_updates = function ()
    local pkgs = { widget = wibox.widget.textbox() }
    function pkgs.update()
        lain_helpers.async({awful.util.shell, "-c", "/home/sebastian/.scripts/statusbar/updates-arch-combined.sh"},
        function(s)
            local markup = string.gsub(s, "\n", "") .. "  "
            pkgs.widget:set_markup(markup)
        end)
    end
    pkgs.timer = lain_helpers.newtimer("package_updates", 60, pkgs.update, false, true)
...
local mympd = lain.widget.mpd {
    ...
    settings = function()
        ...
        if mpd_now.random_mode then
            mode_icon = "󰒝"
        elseif mpd_now.single_mode then
            mode_icon = "󰑘"
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey,           }, "Return", function () awful.spawn(terminal .. " tm-sessions") end,
              {description = "Select a tmux session", group = "launcher"}),
